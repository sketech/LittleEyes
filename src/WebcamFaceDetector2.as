package {
	...
	public class WebcamFaceDetector extends Sprite {
		...
		private var SMC:SMc=new SMc;
		private var Pixels:int=5;
		private var DrawCountX:int=0;
		private var DrawCountY:int=0;
		private var BigPicture:MovieClip=new MovieClip();
		private var _tempMc:MovieClip=new MovieClip();
		private var PictureCount:int=0;
		private var BigPictureState:Boolean=false;
		public function WebcamFaceDetector() {
			...
			//BigPicture.addEventListener(MouseEvent.CLICK, ShowBigPicture);
			//startLoading("0.jpg");
		}
		...
		private function _setupCamera() : void{
			...
			if (camera != null) {
				...
				BigPicture.x=SMC.x+ SMC.width;
				BigPicture.y=SMC.y;
				trace("socesmc.." + SourceBitmapData.width + ".." + SourceBitmapData.height);
			} else {
		...
		private function _detection_complete (event : ObjectDetectorEvent) : void {
			...
			var _drawPixelX,_drawPixelY:int;
			var _isStop:Boolean=false;
			_drawPixelX = DrawCountX;
            _drawPixelY = DrawCountY;
							
			var _SRectangle:Rectangle=new Rectangle(_drawPixelX,_drawPixelY,Pixels,Pixels);
			
			var _Sbmp:Bitmap=GetBimap(new Bitmap( SourceBitmapData ),_SRectangle);
			
			//trace("count.." + _drawPixelX + ".." + _drawPixelY + ".." + GetColor(_Sbmp.bitmapData));
            //addChild(GetColor(_Sbmp.bitmapData));
			var _rectangle1:Rectangle =new Rectangle(_rects[0].x-(_rects[0].width/2),_rects[0].y,_rects[0].width*2,_rects[0].height*2);
			//var _rectangle1:Rectangle =new Rectangle(_rects[0].x-(_rects[0].width/2),_rects[0].y,_rects[0].width*2,_rects[0].height*2);
			var _bmp:Bitmap=GetBimap(_bmpTarget,_rectangle1);
			var _square:Sprite=GetColor(_Sbmp.bitmapData);
			_tempMc["p" + PictureCount ]=new MovieClip();
			addChild(_tempMc["p" + PictureCount]);
  			_bmp.width=_bmp.height=Pixels;
			//_tempMc=new MovieClip();
		    _tempMc["p" + PictureCount].id=PictureCount;
			_tempMc["p" + PictureCount].xx=0;
			_tempMc["p" + PictureCount].yy=0;
			_tempMc["p" + PictureCount].addChild(_bmp);
			_tempMc["p" + PictureCount].x=_video.width+10;
			//_tempMc["p" + PictureCount].y=_video.y;
			
			_square.alpha=.8;
			_tempMc["p" + PictureCount].addChild(_square);
			_tempMc["p" + PictureCount].addEventListener(MouseEvent.CLICK, ShowBigPicture);
			BigPicture.addChild(_tempMc["p" + PictureCount]);
			_tempMc["p" + PictureCount].y=-350+DrawCountX;
			//_tempMc.x=350+DrawCountX;
			
			Tweener.addTween(_tempMc["p" + PictureCount ], {x: DrawCountX , y: DrawCountY, transition: "easeOutBounce",time: .1});
			//Tweener.addTween(BigPicture, {x: SMC.width + DrawCountX , y:SMC.y + DrawCountY, transition: "easeOutBounce",time: .1});
			
			DrawCountX+=Pixels;
			if(DrawCountX >= SourceBitmapData.width){
				DrawCountX=0;
				DrawCountY+=Pixels;
				if(DrawCountY > SourceBitmapData.height) _isStop=true;
			}else{
				//_detector.removeEventListener( ObjectDetectorEvent.DETECTION_COMPLETE , _detection_complete );
			}
			if(_isStop) _detector.removeEventListener( ObjectDetectorEvent.DETECTION_COMPLETE , _detection_complete );
			PictureCount++;
			//Pixels:int=5;
		    //DrawCount:int=0;
			//SourceBitmapData
			...
			//BigPicture.addEventListener(MouseEvent.CLICK, ShowBigPicture);
		private function ShowBigPicture(e:Event):void{
			var _id:int=e.target.parent.id;  
			//_tempMc["p" + PictureCount]
			
			if(BigPicture.scaleX==1) {
				BigPicture.scaleX=BigPicture.scaleY=3;
				BigPicture.y=BigPicture.x=0;
			}else{
				if(_tempMc["p" + _id].width==Pixels && !BigPictureState){
					
					/*e.target.xx=e.target.x;
					e.target.yy=e.target.y;
					e.target.scaleX=e.target.scaleY=20;
					e.target.x=0;
					e.target.y=0;*/
					BigPictureState=true;
					_tempMc["p" + _id].xx=_tempMc["p" + _id].x;
					_tempMc["p" + _id].yy=_tempMc["p" + _id].y;
					_tempMc["p" + _id].scaleX=_tempMc["p" + _id].scaleY=30;
					_tempMc["p" + _id].x=0;
					_tempMc["p" + _id].y=0;
					/*_tempMc["p" + _id].x=(BigPicture.width-_tempMc["p" + _id].width)/2;
					_tempMc["p" + _id].y=(BigPicture.height-_tempMc["p" + _id].height)/2;*/
					//_tempMc["p" + _id].numChildren= numChildren-1;
					BigPicture.setChildIndex(_tempMc["p" + _id],BigPicture.numChildren-1);
					//trace("aaaaaaa.."+ _id + ".." +BigPicture.scaleX  + ".." + _tempMc["p" + _id].width + ".." + BigPictureState );
					/*_tempMc["p" + _id].x=(stage.stageWidth-_tempMc["p" + _id].width)/2;
					_tempMc["p" + _id].y=(stage.stageHeight-_tempMc["p" + _id].height)/2;*/
				}else {
					BigPictureState=false;
					_tempMc["p" + _id].scaleX=_tempMc["p" + _id].scaleY=1;
					_tempMc["p" + _id].x=_tempMc["p" + _id].xx;
					_tempMc["p" + _id].y=_tempMc["p" + _id].yy;
					BigPicture.scaleX=BigPicture.scaleY=1;
					BigPicture.y=SMC.y;
					BigPicture.x=SMC.x+SMC.width;
					//trace("bbbbb.."+ _id + ".." +_tempMc["p" + _id].xx  + ".." + _tempMc["p" + _id].yy + ".." + BigPictureState );
				}
			}
		}
		/**
		...
		private function GetColor(_SourceBitmapData:BitmapData):Sprite{
			var _c:uint;
			var _count:int=0;
			var _bmp:Bitmap;
			...
			for (var i:int=0; i<_SourceBitmapData.width-1; i++) {
      			for (var j:int=0; j<_SourceBitmapData.height-1; j++) {
					_count++;
					var _pixel:Pixel = new Pixel(_SourceBitmapData.getPixel32(i, j));
        			red += _pixel.getRed();
        			green += _pixel.getGreen();
        			blue += _pixel.getBlue();
      			}
			}
